...
static void handle_events(struct pc_context *pctx)
{
	...
	while (p) {
		...
		case event_type_micgain:
			...
			break;
/* AEC */
		case event_type_echocancel:
			snprintf(msgbuf, MBS, "AEC: %s%d",
                        pctx->aec_type == 0 ? "off param: " : pctx->aec_type == 1 ? "speex filter_len: " : "webRTC latency: ", pctx->aec_param);
			msgbook_enqueue(&mb0, MB_TYPE_GUI, MODULE, msgbuf);
			break;
/* AEC */
...
/* AEC */
	case 'E':
		ret = pc_engine_cmd_set_echocancel(pctx, cmd_aec_type, cmd_aec_param);
		if (ret == -1)
			msgbook_enqueue(&mb0, MB_TYPE_ERROR, MODULE, "Could not set AEC algorithm");
		break;
/* AEC */
	case 'g':
...
/* AEC */
		case 'E':
			options.aec_algo = atoi(optarg);
			break;
		case 'e':
			options.aec_param = atoi(optarg);
			break;
/* AEC */
		case 'c':
			free(options.autocall_host);
...
/* AEC */
        ret = pc_engine_cmd_set_echocancel(&pctx, options.aec_algo, options.aec_param);
	if (ret == -1)
		die("pc_engine_cmd_set_echocancel() failed", 1);
/* AEC */
