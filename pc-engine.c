...
#include "speex_aec.h"
int16_t* farData  = NULL;
int16_t* nearData = NULL;
int16_t* ecData   = NULL;
/* AEC */
...
/* AEC */
static void pc_engine_aec_far_end(struct pc_context *pctx) {
// pctx->lb_pcm (mikrofon)
// pctx->pcm (reprak)
// channels by malo byt VZDY 2 (predpoklada to adsp.c ?)
    unsigned i;

    if (pctx->aec_type == 0) return;

    if (!farData || !nearData || (pctx->aec_type == 1 && !ecData)) {
        snprintf(msgbuf, MBS, "%s(): Ooops...AEC buffers not initialized", __func__);
        msgbook_enqueue(&mb0, MB_TYPE_DEBUG, MODULE, msgbuf);
        return;
    }
...
//int delay = 93; //rpi
//int delay = 260; //simcom
static void pc_engine_aec_near_end(struct pc_context *pctx) {
    unsigned i;
    ...
    if (pctx->aec_type == 1) {
        ...
        speex_aecProcessNearEnd();

        for (i = 0; i < ONE_OPUS_FRAME; i++) { // copy result
            pctx->lb_pcm[2*i] = pctx->lb_pcm[2*i + 1] = ecData[i];
        ...
        for (i = 0; i < ONE_OPUS_FRAME/2; i++)  // copy result 1st half
            pctx->lb_pcm[2*i] = pctx->lb_pcm[2*i + 1] = nearData[i];

        for (i = ONE_OPUS_FRAME/2; i < ONE_OPUS_FRAME; i++) // copy 2nd half (160 samples, 10ms) mono signal to EC
            nearData[i - ONE_OPUS_FRAME/2] = (int16_t)((pctx->lb_pcm[2*i] + pctx->lb_pcm[2*i + 1])>>1);
...
*/
	...
	*nfds += pctx->ad_capture.nfds;
	...
	//pctx->bandwidth = OPUS_BANDWIDTH_SUPERWIDEBAND;
...
#if 0
static void printTime(char* name) {
    struct timespec time;

    clock_gettime(CLOCK_MONOTONIC_RAW, &time);
    fprintf(stderr, "[%lld.%03ld] %s\n", (long long)time.tv_sec, time.tv_nsec/1000000, name);
}
#endif
...
/* AEC */
                if (hasPlayed && pctx->aec_type != 0) pc_engine_aec_near_end(pctx);
                hasPlayed = 0;
/* AEC */
...
//			if ((1+pctx->ic_total) % 50 == 0) { /* nop packets only every 1s */
        ...
        if (mix_audio(pctx)) {
/* AEC */
                if (!pctx->micmute && pctx->aec_type != 0) pc_engine_aec_far_end(pctx);
                hasPlayed = 1;
/* AEC */
                // playback decoded data
//printTime("beforeWrite");  // na RPi2 to od `afterRead` sem trva 5ms. Cela fcn pc_engine_audio_ready() sa vola kazdych `period` ms
...
/* AEC */
int pc_engine_cmd_set_echocancel(struct pc_context *pctx, int type, int param)
{
        if (type < 0 || type > 2 || param < 0 || param > 1000000)
            return -1;

        if (pctx->aec_type != type) {         // AEC type change
            if (pctx->aec_type == 1) {        // speex
                speex_aecDeinit();
                if (*pctx->verbose >= 2) {
                    snprintf(msgbuf, MBS, "speex AEC deinitialized");
                    msgbook_enqueue(&mb0, MB_TYPE_DEBUG, MODULE, msgbuf);
                }
            } else if (pctx->aec_type == 2) { // webRTC
                webRTC_aecDeinit();
                if (*pctx->verbose >= 2) {
                    snprintf(msgbuf, MBS, "webRTC AEC deinitialized");
                    msgbook_enqueue(&mb0, MB_TYPE_DEBUG, MODULE, msgbuf);
                }
            }
            ...
            pctx->aec_type = type;
            pctx->aec_param = param;
        }

        if (pctx->aec_param != param) {
            if (pctx->aec_type == 2) {
                if (*pctx->verbose >= 2) {
                    snprintf(msgbuf, MBS, "webRTC AEC latency changed to %dms", param);
                    msgbook_enqueue(&mb0, MB_TYPE_DEBUG, MODULE, msgbuf);
                }
                pctx->aec_param = param;
            } else if (pctx->aec_type == 1) {
                snprintf(msgbuf, MBS, "speex AEC filter length can only be changed during initialization");
                msgbook_enqueue(&mb0, MB_TYPE_DEBUG, MODULE, msgbuf);
            }
        }

	event_push(&pctx->events, event_type_echocancel);
        return 0;
}
/* AEC */
